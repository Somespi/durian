...
pub fn introduce(cells: c_int, i_height: c_int, i_width: c_int) Grid {
    ...
    const fCells: f64 = @floatFromInt(cells);
...
pub fn getPositionedGrid(self: Grid, points: []Point) anyerror!Rectangle {
    ...
    const maxCol: f64 = @floatFromInt(points[points.len - 1][0]);
    const maxRow: f64 = @floatFromInt(points[points.len - 1][1]);
    const minCol: f64 = @floatFromInt(points[0][0]);
    const minRow: f64 = @floatFromInt(points[0][1]);
